...
struct ServoConfig {
  ...
  bool reversed;       // true if this servo is mounted so its physical
                        // rotation is mirrored (90 stays 90, but every
                        // other angle needs to be flipped: 180 - angle)
};

// Add/remove/edit rows here. Order doesn't matter.
ServoConfig servoConfigs[] = {
  { "eyeLeft",     2, 90,  30, 150, false },
  { "eyeRight",    3, 90,  30, 150, false },
  { "eyelidLeft",  4, 90,  60, 120, false },
  { "eyelidRight", 5, 90,  60, 120, false },
  { "jaw",         6, 90,  15, 120, true  }, // mounted reversed on the new servo
  { "neck1",       7, 90,  30, 150, false },
  { "neck2",       8, 90,  30, 150, false },
  { "neck3",       9, 90,  30, 150, false },
};
...
// Tracks the last commanded LOGICAL angle for each servo (i.e. the angle
// your code thinks it's at). This is separate from what's actually written
// to a reversed servo, so animations never need to know or care which
// servos are mounted reversed.
...
// Angle is always the LOGICAL angle (matching your config's min/max/home) —
// if the servo is marked reversed, this flips it before writing to the
// actual hardware, so every animation elsewhere in the sketch is unaffected.
void moveServo(const char* name, int angle) {
  ...
  int physicalAngle = servoConfigs[idx].reversed ? (180 - angle) : angle;
  servos[idx].write(physicalAngle);
}
...
void rorAnimation() {
  ...
  const int stepDelayMs = 20; // same slightly-slower-than-normal pace as blink
  ...
  const int wobbleStepDelayMs = 7;  // shorter delay keeps total speed about the same
  ...
  const int returnStepDelayMs = 6; // shorter delay keeps total speed the same
